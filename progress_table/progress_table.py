...
class ProgressTable:
    ...
    def __init__(
        ...
        self.request_header = True
        self.request_splitter = False
    ...
    def add_column(
        ...
        *,
        ...
        if width is None and resolved_width < len(str(name)):
    ...
    def update(self, key, value, *, weight=1):
        ...
        self.new_row.setdefault(key, 0)
        self.new_row_cumulated_weight.setdefault(key, 0)

        fn = self.column_aggregates[key]
        self.new_row[key] = fn(value, self.new_row[key], self.new_row_cumulated_weight[key])
        self.new_row_cumulated_weight[key] += weight

        t0 = time.time()
        td = t0 - self._last_time_row_printed
        if self.is_table_opened and self.print_row_on_update and td > 1 / self.refresh_rate:
    ...
    def print_bar(self, left: str, center: str, right: str):
        ...
        self.print("".join(content), end="\n")

    def print_bar_top(self):
        return self.print_bar(self.table_style.down_right, self.table_style.no_up, self.table_style.down_left)

    def print_bar_bot(self):
        return self.print_bar(self.table_style.up_right, self.table_style.no_down, self.table_style.up_left)

    def print_bar_mid(self):
        return self.print_bar(self.table_style.no_left, self.table_style.all, self.table_style.no_right)

    def get_row_str(self):
        content = []
        for column in self.column_names:
            value = self.new_row.get(column, "")
            value = self.custom_format(value)
            value = self.apply_cell_formatting(str_value=str(value), column_name=column)
    ...
    def next_row(self, colors: ColorFormat | Dict[str, ColorFormat] = None, split=False, header=False):
        if header or self.request_header or self.previous_header_counter >= self.reprint_header_every_n_rows:
            self.print_header()
            split = False
        if split or self.request_splitter:
            self.print_splitter()

        self.prepare_row_color_dict(colors)
        self.display_new_row()
        self.prepare_row_color_dict()
        self.print()  # Insert a newline
    ...
    def apply_cell_formatting(self, str_value: str, column_name: str):
    ...
    def print_header(self):
        ...
        self.request_header = False
        self.request_splitter = False
        self.is_table_opened = True
        self.previous_header_counter = 0
        self.print_bar_mid()

    def print_row_with_progress_bar(self, step, total, infobar, embedded):
    ...
    def __call__(self, iterator: int | Iterable, *range_args):
        ...
        for idx, element in enumerate(iterator):
            if time.time() - t_last_printed > 1 / self.refresh_rate:
                ...
                self._refresh_progress_bar = lambda: self.print_row_with_progress_bar(
                    idx,
                    iterator_length,
                    infobar=infobar,
